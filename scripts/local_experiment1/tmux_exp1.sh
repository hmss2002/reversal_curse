...
# 为了提升“正向(d2p) 描述->名字”的学习效果，默认给更充足的收敛预算。
NUM_EPOCHS="${NUM_EPOCHS:-6}"
# [User Request] 强制禁用环境变量继承，默认不使用 token_budget
TOKEN_BUDGET=""
MAX_SEQ_LEN="${MAX_SEQ_LEN:-512}"
# 提高 GPU 利用率：在显存允许的情况下增大 micro batch。
# DDP world_size=4 时，global_batch = MICRO_BS * GRAD_ACCUM * 4
MICRO_BS="${MICRO_BS:-12}"
GRAD_ACCUM="${GRAD_ACCUM:-1}"
MIXED_PRECISION="${MIXED_PRECISION:-fp16}"
MAX_GRAD_NORM="${MAX_GRAD_NORM:-1.0}"

# LoRA 容量：默认 r=64（更强容量，通常对记映射/记实体名更有帮助），alpha 同步放大。
LORA_R="${LORA_R:-64}"
LORA_ALPHA="${LORA_ALPHA:-128}"
LORA_DROPOUT="${LORA_DROPOUT:-0.05}"

# Eval batch：增大可以提升吞吐（更多样本并行解码），但可能 OOM；不稳就调小。
EVAL_BS="${EVAL_BS:-48}"

# Eval 生成长度：名字任务很短；描述任务需要更长生成预算。
MAX_NEW_TOKENS_NAME="${MAX_NEW_TOKENS_NAME:-12}"
MAX_NEW_TOKENS_DESC="${MAX_NEW_TOKENS_DESC:-64}"

# 评测：按任务类型分别约束（名字 vs 描述）。
# - d2p / p2d_reverse: 期望输出人名
# - p2d / d2p_reverse / both: 期望输出描述
FORMAT_INSTR_NAME="${FORMAT_INSTR_NAME:-\n\nAnswer with only the persons name. Do not explain.\n}"
FORMAT_INSTR_DESC="${FORMAT_INSTR_DESC:-\n\nAnswer with only the description. Do not add extra commentary.\n}"
...
TRAIN_COMMON="$STOP_ARG --max_seq_len $MAX_SEQ_LEN --micro_batch_size $MICRO_BS --grad_accum_steps $GRAD_ACCUM --mixed_precision $MIXED_PRECISION --max_grad_norm $MAX_GRAD_NORM --lora_r $LORA_R --lora_alpha $LORA_ALPHA --lora_dropout $LORA_DROPOUT"

TMUX_CMD_PHI="set -o pipefail; cd $ROOT_DIR && \
  mkdir -p $PHI_DIR && \
  echo '[phi] start' && date && \
  if [[ -f $PHI_DIR/adapter_model.safetensors ]]; then echo '[phi] adapter found, skipping training'; else \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29501 scripts/local_experiment1/train_lora.py --preset phi3_5_mini_instruct $TRAIN_COMMON --output_dir $PHI_DIR 2>&1 | tee $PHI_DIR/train.log; fi && \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29501 scripts/local_experiment1/eval_experiment1.py --base_model_id microsoft/Phi-3.5-mini-instruct --revision 2fe192450127e6a83f7441aef6e3ca586c338b77 --lora_dir $PHI_DIR --dataset_dir data/reverse_experiments/june_version_7921032488 --out_dir $PHI_DIR/eval --format_instruction_name \"$FORMAT_INSTR_NAME\" --format_instruction_description \"$FORMAT_INSTR_DESC\" --max_new_tokens_name $MAX_NEW_TOKENS_NAME --max_new_tokens_description $MAX_NEW_TOKENS_DESC --batch_size $EVAL_BS 2>&1 | tee $PHI_DIR/eval.log && \
  date > $PHI_DONE && \
  printf '\\a' && tmux display-message -d 0 '[DONE] phi finished' && \
  echo '[phi] done' && date"

TMUX_CMD_QWEN="set -o pipefail; cd $ROOT_DIR && \
  mkdir -p $QWEN_DIR && \
  echo '[qwen] waiting for: $PHI_DONE' && \
  while [[ ! -f $PHI_DONE ]]; do sleep 30; date; done && \
  echo '[qwen] start' && date && \
  if [[ -f $QWEN_DIR/adapter_model.safetensors ]]; then echo '[qwen] adapter found, skipping training'; else \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29502 scripts/local_experiment1/train_lora.py --preset qwen3_4b $TRAIN_COMMON --output_dir $QWEN_DIR 2>&1 | tee $QWEN_DIR/train.log; fi && \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29502 scripts/local_experiment1/eval_experiment1.py --base_model_id Qwen/Qwen3-4B --revision 1cfa9a7208912126459214e8b04321603b3df60c --lora_dir $QWEN_DIR --dataset_dir data/reverse_experiments/june_version_7921032488 --out_dir $QWEN_DIR/eval --format_instruction_name \"$FORMAT_INSTR_NAME\" --format_instruction_description \"$FORMAT_INSTR_DESC\" --max_new_tokens_name $MAX_NEW_TOKENS_NAME --max_new_tokens_description $MAX_NEW_TOKENS_DESC --batch_size $EVAL_BS 2>&1 | tee $QWEN_DIR/eval.log && \
  date > $QWEN_DONE && \
  printf '\\a' && tmux display-message -d 0 '[DONE] qwen finished' && \
  echo '[qwen] done' && date"

TMUX_CMD_GEMMA="set -o pipefail; cd $ROOT_DIR && \
  mkdir -p $GEMMA_DIR && \
  echo '[gemma] waiting for: $QWEN_DONE' && \
  while [[ ! -f $QWEN_DONE ]]; do sleep 30; date; done && \
  echo '[gemma] start' && date && \
  if [[ -f $GEMMA_DIR/adapter_model.safetensors ]]; then echo '[gemma] adapter found, skipping training'; else \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29503 scripts/local_experiment1/train_lora.py --preset gemma3_4b $TRAIN_COMMON --output_dir $GEMMA_DIR 2>&1 | tee $GEMMA_DIR/train.log; fi && \
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TORCHRUN --nproc_per_node $TORCHRUN_NPROC --master_port 29503 scripts/local_experiment1/eval_experiment1.py --base_model_id google/gemma-3-4b-it --revision 093f9f388b31de276ce2de164bdc2081324b9767 --lora_dir $GEMMA_DIR --dataset_dir data/reverse_experiments/june_version_7921032488 --out_dir $GEMMA_DIR/eval --format_instruction_name \"$FORMAT_INSTR_NAME\" --format_instruction_description \"$FORMAT_INSTR_DESC\" --max_new_tokens_name $MAX_NEW_TOKENS_NAME --max_new_tokens_description $MAX_NEW_TOKENS_DESC --batch_size $EVAL_BS 2>&1 | tee $GEMMA_DIR/eval.log && \
  date > $GEMMA_DONE && \
...
PROGRESS_MODE="epoch"
PROGRESS_VALUE="$NUM_EPOCHS"
if [[ -n "$TOKEN_BUDGET" ]]; then
  PROGRESS_MODE="token"
  PROGRESS_VALUE="$TOKEN_BUDGET"
fi
PROGRESS_CMD="cd $ROOT_DIR && bash scripts/local_experiment1/watch_progress.sh $RUN_ROOT $PROGRESS_MODE $PROGRESS_VALUE"
